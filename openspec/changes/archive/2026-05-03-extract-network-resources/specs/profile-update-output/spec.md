...
## Purpose
Define the output format for profile update operations, ensuring that update results include per-resource status with URL privacy (domain-only display, not full raw URLs).
