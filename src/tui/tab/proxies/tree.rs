...
mod tests {
    ...
    fn tree_builds_all_top_level_groups() {
        ...
        let tree = ProxyTree::build(ProxiesResponse { proxies: proxies.clone() });
        let top_count = proxies
            .iter()
            .filter(|(_, p)| !p.hidden && p.all.as_ref().map(|a| !a.is_empty()).unwrap_or(false))
            .count();
        let folder_count = tree.nodes.iter().filter(|n| n.node_type == NodeType::Folder).count();
        assert_eq!(folder_count, top_count, "Every top-level group becomes a Folder");
    }
    ...
    fn zero_delay_history_shows_fail() {
        let proxies = load_fixture();
        assert_eq!(resolve_delay("DIRECT", &proxies), Some(0), "All-zero history = FAIL");
    }
    ...
    fn cycle_detection_returns_none() {
        ...
        proxies.insert("A".to_string(), Proxy {
            name: "A".to_string(),
            now: Some("B".to_string()),
            all: Some(vec!["B".to_string()]),
            ..Default::default()
        });
        proxies.insert("B".to_string(), Proxy {
            name: "B".to_string(),
    ...
    fn node_types_are_correct() {
        ...
        for node in &tree.nodes {
            match node.node_type {
                NodeType::Folder => assert!(node.parent.is_none(), "Folder {} should be top-level", node.name),
                NodeType::Link | NodeType::File => {
                    assert!(node.parent.is_some(), "{:?} {} should have parent", node.node_type, node.name);
                    assert!(node.depth > 0, "{:?} {} should be nested", node.node_type, node.name);
                }
            }
        }
    }
    ...
    fn expanded_folder_has_nested_children() {
        let proxies = load_fixture();
        let child_count = proxies.get("Entry").unwrap().all.as_ref().map(|a| a.len()).unwrap();
    ...
    fn empty_history_returns_none() {
        let mut proxies = IndexMap::new();
        proxies.insert("empty".to_string(), Proxy {
    ...
    fn zero_delay_produces_fail() {
        let mut proxies = IndexMap::new();
        proxies.insert("dead".to_string(), Proxy {
            name: "dead".to_string(),
            proxy_type: "Vmess".to_string(),
    ...
    fn now_chain_multiple_levels() {
        let mut proxies = IndexMap::new();
        proxies.insert("G1".to_string(), Proxy {
            name: "G1".to_string(), proxy_type: "Selector".to_string(),
            now: Some("G2".to_string()), all: Some(vec!["G2".to_string()]),
            ..Default::default()
        });
        proxies.insert("G2".to_string(), Proxy {
            name: "G2".to_string(), proxy_type: "Selector".to_string(),
            now: Some("LEAF".to_string()), all: Some(vec!["LEAF".to_string()]),
            ..Default::default()
        });
        proxies.insert("LEAF".to_string(), Proxy {
            name: "LEAF".to_string(), proxy_type: "Vmess".to_string(),
    ...
    fn now_chain_self_ref_stops() {
        let mut proxies = IndexMap::new();
        proxies.insert("self-ref".to_string(), Proxy {
            name: "self-ref".to_string(), proxy_type: "Selector".to_string(),
            now: Some("self-ref".to_string()), all: Some(vec!["self-ref".to_string()]),
